$(document).ready(function () {
	...
	if ($("#form_dossier").val() && isStart) {
		isStart = false;
		let ref_individu = $("#form_dossier").val();
       let params = ['20_130_0', '20_130_3', '0',ref_individu];
        let paramJson = JSON.stringify(params);
        let urlAppele = $('#routeRouteur').val() + '?params=' + paramJson;
	...
	//----------------------------------------------------------------------
	// champ form 13-classement bouton down
	...
	$('#edit_doc').click(function () { // section 
				let context = tPage;
		let contextJson = JSON.stringify(context);
		let params = [$('#nodeCourant').val(), "20_130_0", "5", contextJson];
		let paramJson = JSON.stringify(params);
		let href = $("#routeRouteur").val() + "?params=" + paramJson;
		window.open(href, '_self');
	});
