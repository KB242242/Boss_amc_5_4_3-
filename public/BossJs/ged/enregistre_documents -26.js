...
// 0-Declaration de la table
	...
	let params = [nodeCourant, tableAjax, []];
...
//$('#debug_resultat').html('SALUT-1'); // Afficher le message dans l'élément avec l'id "resultat"
			...
			$.ajax({
				...
				success: function(response) {
					...
					$('#debug_resultat').html(response); // Afficher le message dans l'élément avec l'id "resultat"
	...
	$('#enregistreDocument').click(function () { // section 110-reserve_documents
		let context = [];
		let contextJson = JSON.stringify(context);
		let params = [$('#nodeCourant').val(), "20_120_0", contextJson];
		let paramJson = JSON.stringify(params);
		let href = $("#routeRouteur").val() + "?params=" + paramJson;
		window.open(href, '_self');
	});
